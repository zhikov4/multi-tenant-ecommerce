...
<template>
    ...
    <div class="min-h-screen bg-slate-50 p-8 font-sans">
        <div class="max-w-6xl mx-auto">
            ...
            <div v-if="$page.props.flash?.success" class="bg-green-50 border border-green-200 text-green-700 font-bold px-5 py-3 rounded-2xl mb-6">
                {{ $page.props.flash.success }}
            </div>

            <div v-if="products.data.length === 0" class="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-slate-200">
                <svg class="w-16 h-16 text-slate-200 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4"/></svg>
            ...
            <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                <div v-for="product in products.data" :key="product.id"
                    class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm group">
                    <div class="flex justify-between items-start mb-4">
                        <span :class="product.is_active ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'"
                    ...
                    <Link :href="route('products.edit', product.id)"
                        class="block w-full text-center bg-slate-50 text-slate-700 py-2.5 rounded-2xl font-bold hover:bg-blue-50 hover:text-blue-700 transition-all border border-slate-200">
                        Edit Product
                    </Link>
                </div>
            </div>
