...
const addToCart = () => {
    if (props.auth && props.auth.user) {
        alert('Yeay! Item added to cart 🛒 (Database integration pending)');
...
// FIX: Tambahin hard redirect biar sesinya refresh sempurna setelah logout
const submitLogin = () => { 
...
<template>
    ...
    <div class="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between break-words">
                ...
                <div class="flex items-center gap-2 sm:gap-4">
                    <template v-if="$page.props.auth.user">
                        <Link :href="route('dashboard')" class="bg-slate-900 text-white px-4 sm:px-6 py-2 sm:py-2.5 rounded-xl font-bold shadow-lg hover:bg-blue-600 transition-all text-sm sm:text-base text-center break-words">Dashboard</Link>
                    </template>
                    <template v-else>
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
            <h1 class="text-3xl sm:text-4xl font-black mb-8 text-slate-900 break-words">Marketplace Discovery</h1>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
                <div v-for="product in products" :key="product.id" 
                    ...
                    <p class="text-[10px] font-black uppercase tracking-widest text-blue-500 mb-1 break-words">{{ product.store }}</p>
                    <h3 class="font-bold text-lg text-slate-800 mb-2 break-words line-clamp-2">{{ product.name }}</h3>
                    <p class="text-xl font-black text-slate-900 mt-auto break-words">Rp {{ product.price.toLocaleString('id-ID') }}</p>
                </div>
            </div>
        </main>
            ...
            <div class="bg-white w-full max-w-4xl rounded-[2rem] sm:rounded-[3rem] overflow-hidden shadow-2xl flex flex-col md:flex-row animate-in zoom-in duration-300 max-h-[95vh] overflow-y-auto">
                <div class="md:w-1/2 bg-slate-50 p-6 sm:p-10 flex flex-col justify-between border-b md:border-b-0 md:border-r border-slate-100 min-w-0">
                    <div class="break-words">
                        <div class="aspect-square bg-white rounded-3xl border border-slate-100 flex items-center justify-center mb-6 shadow-sm flex-shrink-0">
                        ...
                        <h2 class="text-2xl sm:text-3xl font-black text-slate-900 mb-2 break-words">{{ selectedProduct.name }}</h2>
                        <p class="text-blue-600 font-bold text-sm tracking-wide mb-4 break-words">STORE: {{ selectedProduct.store }}</p>
                        <div class="prose prose-sm text-slate-500 break-words">
                ...
                <div class="md:w-1/2 p-6 sm:p-10 flex flex-col relative min-w-0">
                    ...
                    <h3 class="text-lg sm:text-xl font-black text-slate-900 mb-4 sm:mb-6 border-b border-slate-100 pb-4 break-words pr-8">Purchase Details</h3>
                    
                    <div class="space-y-4 sm:space-y-5 flex-1 break-words">
                        <div>
                            <p class="text-xs font-black text-slate-400 uppercase tracking-widest mb-1">Unit Price</p>
                            <p class="text-2xl sm:text-3xl font-black text-blue-600 break-words">Rp {{ selectedProduct.price.toLocaleString('id-ID') }}</p>
                        </div>
                        <div>
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 break-words">Select Variant/Size</label>
                            <select v-model="purchaseForm.variant" class="w-full mt-1 bg-slate-50 border-slate-200 rounded-2xl py-3 px-4 font-bold focus:ring-blue-500 outline-none break-words">
                        ...
                        <div>
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 break-words">Quantity</label>
                        ...
                        <div>
                            <label class="text-xs font-black text-slate-400 uppercase tracking-widest ml-1 break-words">Notes for Seller</label>
                            <textarea v-model="purchaseForm.note" rows="2" placeholder="e.g. Please pack safely..." class="w-full mt-1 bg-slate-50 border-slate-200 rounded-2xl py-3 px-4 font-bold focus:ring-blue-500 outline-none break-words"></textarea>
                        </div>
                    </div>
                    
                    <div class="pt-4 sm:pt-6 border-t border-slate-100 mt-4 sm:mt-6 flex flex-col sm:flex-row gap-3 sm:gap-4">
                        <button @click="addToCart" class="flex-1 bg-white text-slate-700 py-3 sm:py-4 rounded-2xl font-black hover:bg-slate-50 transition-all border-2 border-slate-200 break-words text-sm sm:text-base">
                            Add to Cart
                        </button>
                        <button @click="proceedToCheckout" class="flex-1 bg-blue-600 text-white py-3 sm:py-4 rounded-2xl font-black hover:bg-blue-700 shadow-lg shadow-blue-200 transition-all flex justify-center items-center gap-2 break-words text-sm sm:text-base">
                            Buy Now 🛒
                        </button>
                    </div>
                </div>
        ...
        <div v-if="isCheckoutModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md" @click.self="isCheckoutModalOpen = false">
            <div class="bg-white w-full max-w-xl rounded-[2rem] sm:rounded-[3rem] p-6 sm:p-10 shadow-2xl animate-in zoom-in duration-300 max-h-[95vh] overflow-y-auto min-w-0">
                <div class="flex justify-between items-center mb-6 break-words">
                    <h2 class="text-xl sm:text-2xl font-black text-slate-900 flex items-center gap-2 break-words pr-4">
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 sm:p-5 mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-4 break-words min-w-0">
                    <div class="min-w-0">
                        <p class="text-[10px] sm:text-xs font-bold text-blue-600 uppercase tracking-widest mb-1 break-words">Order Summary</p>
                        <p class="font-black text-slate-800 break-words line-clamp-2">{{ selectedProduct.name }} (x{{ purchaseForm.quantity }})</p>
                    </div>
                ...
                <form @submit.prevent="confirmOrder" class="space-y-4 sm:space-y-5 break-words min-w-0">
                    <div class="min-w-0">
                        <label class="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest ml-1 break-words">Recipient Name</label>
                        <input v-model="checkoutForm.fullName" type="text" class="w-full mt-1 bg-slate-50 border-slate-200 rounded-2xl py-3 px-4 font-bold focus:ring-blue-500 outline-none break-words min-w-0" required />
                    </div>
                    <div class="min-w-0">
                        <label class="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest ml-1 break-words">Full Delivery Address</label>
                        <textarea v-model="checkoutForm.address" rows="3" placeholder="Street, City, Zip Code..." class="w-full mt-1 bg-slate-50 border-slate-200 rounded-2xl py-3 px-4 font-bold focus:ring-blue-500 outline-none break-words min-w-0" required></textarea>
                    </div>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
                        <div class="min-w-0">
                            <label class="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest ml-1 break-words">Shipping</label>
                            <select v-model="checkoutForm.shipping" class="w-full mt-1 bg-slate-50 border-slate-200 rounded-2xl py-3 px-4 font-bold focus:ring-blue-500 outline-none break-words min-w-0">
                        ...
                        <div class="min-w-0">
                            <label class="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest ml-1 break-words">Payment Method</label>
                            <select v-model="checkoutForm.payment" class="w-full mt-1 bg-slate-50 border-slate-200 rounded-2xl py-3 px-4 font-bold focus:ring-blue-500 outline-none break-words min-w-0">
                    ...
                    <button type="submit" class="w-full bg-green-600 text-white py-3 sm:py-4 rounded-2xl font-black hover:bg-green-700 shadow-lg shadow-green-200 transition-all mt-6 text-base sm:text-lg break-words">
                        Confirm & Pay Now 💳
                    </button>
                </form>
            </div>
        ...
        <div v-if="showLoginModal" class="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" @click.self="showLoginModal = false">
            <div class="bg-white w-full max-w-md rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-2xl animate-in zoom-in duration-300 min-w-0">
                <div class="flex justify-between items-center mb-4 sm:mb-6 break-words">
                    <h2 class="text-2xl sm:text-3xl font-black text-slate-900 flex items-center gap-2 break-words">
                ...
                <p class="text-slate-500 text-sm mb-6 font-medium break-words">Please sign in to continue.</p>

                <form @submit.prevent="submitLogin" class="space-y-4 min-w-0">
                    <div>
                        <input v-model="loginForm.email" type="email" placeholder="Email" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-3 px-5 font-bold focus:ring-blue-500 outline-none break-words min-w-0" required />
                        <p v-if="loginForm.errors.email" class="text-red-500 text-xs mt-1 ml-2 font-bold">{{ loginForm.errors.email }}</p>
                    </div>
                    
                    <div>
                        <input v-model="loginForm.password" type="password" placeholder="Password" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-3 px-5 font-bold focus:ring-blue-500 outline-none break-words min-w-0" required />
                        <p v-if="loginForm.errors.password" class="text-red-500 text-xs mt-1 ml-2 font-bold">{{ loginForm.errors.password }}</p>
                    </div>
                    
                    <button type="submit" :disabled="loginForm.processing" class="w-full bg-slate-900 text-white py-3 sm:py-4 rounded-2xl font-black hover:bg-blue-600 transition-all mt-4 break-words flex justify-center items-center gap-2">
                ...
                <div class="mt-6 text-center border-t border-slate-100 pt-6 min-w-0">
                    <p class="text-sm font-bold text-slate-500 break-words flex flex-col sm:flex-row justify-center items-center gap-1 sm:gap-2">
                        Don't have an account? 
                        <button @click="switchToRegister" class="text-blue-600 hover:text-blue-800 break-words">Sign Up Now</button>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div v-if="showRegisterModal" class="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md" @click.self="showRegisterModal = false">
            <div class="bg-white w-full max-w-md rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-8 shadow-2xl animate-in zoom-in duration-300 max-h-[90vh] overflow-y-auto min-w-0">
                ...
                <p class="text-slate-500 text-sm mb-6 font-medium break-words">Create a new account. It's quick and free.</p>

                <form @submit.prevent="submitRegister" class="space-y-4 min-w-0">
                    <div>
                        <input v-model="registerForm.name" type="text" placeholder="Name" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-3 px-5 font-bold focus:ring-blue-500 outline-none break-words min-w-0" required />
                        <p v-if="registerForm.errors.name" class="text-red-500 text-xs mt-1 ml-2 font-bold">{{ registerForm.errors.name }}</p>
                    </div>
                    <div>
                        <input v-model="registerForm.email" type="email" placeholder="Email" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-3 px-5 font-bold focus:ring-blue-500 outline-none break-words min-w-0" required />
                        <p v-if="registerForm.errors.email" class="text-red-500 text-xs mt-1 ml-2 font-bold">{{ registerForm.errors.email }}</p>
                    </div>
                    <div>
                        <input v-model="registerForm.password" type="password" placeholder="Password" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-3 px-5 font-bold focus:ring-blue-500 outline-none break-words min-w-0" required />
                        <p v-if="registerForm.errors.password" class="text-red-500 text-xs mt-1 ml-2 font-bold">{{ registerForm.errors.password }}</p>
                    </div>
                    <div>
                        <input v-model="registerForm.password_confirmation" type="password" placeholder="Confirm Password" class="w-full bg-slate-50 border-slate-200 rounded-2xl py-3 px-5 font-bold focus:ring-blue-500 outline-none break-words min-w-0" required />
                    ...
                    <button type="submit" :disabled="registerForm.processing" class="w-full bg-blue-600 text-white py-3 sm:py-4 rounded-2xl font-black hover:bg-blue-700 transition-all mt-4 break-words flex justify-center items-center gap-2">
                ...
                <div class="mt-6 text-center border-t border-slate-100 pt-6 min-w-0">
                    <p class="text-sm font-bold text-slate-500 break-words flex flex-col sm:flex-row justify-center items-center gap-1 sm:gap-2">
                        ...
                        <button @click="switchToLogin" class="text-blue-600 hover:text-blue-800 break-words">Sign In</button>
                    </p>
                </div>
            </div>
        </div>
